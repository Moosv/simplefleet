...
export default function ManagerStatsPage() {
  ...
  const { data: records } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        ...
        .select('*, employees(name, teams(name)), vehicles(name, license_plate)')
      if (dateFilter) {
  ...
  // 실별
  const teamChartData = Object.entries(
  ...
  const noData = <div className="flex items-center justify-center text-sm text-gray-400" style={{ height: 200 }}>데이터 없음</div>
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {teamChartData.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 p-5 mb-4">
          <h2 className="text-sm font-semibold text-gray-800 mb-1">실별 운행 현황</h2>
          <p className="text-xs text-gray-400 mb-4">왼쪽 축: 거리(km) &nbsp;·&nbsp; 오른쪽 축: 건수(회)</p>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={teamChartData} margin={{ top: 5, right: 40, left: 0, bottom: 5 }}>
              ...
              <Legend />
              <Bar yAxisId="left" dataKey="거리" fill="#ec4899" radius={[4, 4, 0, 0]} />
              <Bar yAxisId="right" dataKey="건수" fill="#8b5cf6" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {vehicleChartData.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 p-5 mb-4">
          ...
          <p className="text-xs text-gray-400 mb-4">왼쪽 축: 거리(km) &nbsp;·&nbsp; 오른쪽 축: 건수(회)</p>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={vehicleChartData} margin={{ top: 5, right: 40, left: 0, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" />
              <XAxis dataKey="name" tick={{ fontSize: 11 }} />
              <YAxis yAxisId="left" orientation="left" tick={{ fontSize: 11 }} unit="km" />
              <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 11 }} unit="회" allowDecimals={false} />
              <Tooltip formatter={(value, name) => name === '거리' ? [`${Number(value).toLocaleString()}km`, '거리'] : [`${value}회`, '건수']} />
              <Legend />
              <Bar yAxisId="left" dataKey="거리" fill="#06b6d4" radius={[4, 4, 0, 0]} />
              <Bar yAxisId="right" dataKey="건수" fill="#f59e0b" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <p className="text-xs text-gray-400 mb-3">왼쪽: 거리 &nbsp;·&nbsp; 오른쪽: 건수</p>
          {driverChartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={driverChartData} margin={{ top: 5, right: 35, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" />
                <XAxis dataKey="name" tick={{ fontSize: 11 }} />
                <YAxis yAxisId="left" orientation="left" tick={{ fontSize: 11 }} unit="km" />
                <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 11 }} unit="회" allowDecimals={false} />

...
export default function StatsPage() {
  ...
  // 부서별 집계
  const deptStats = records?.reduce<Record<string, { 건수: number; 거리: number }>>((acc, r) => {
    const name = (r as typeof r & { departments?: { name: string } | null }).departments?.name ?? '미지정'
  ...
  const deptChartData = Object.entries(deptStats ?? {}).map(([name, v]) => ({ name, ...v }))
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-5 mb-4">
        <h2 className="text-sm font-semibold text-gray-800 mb-4">부서별 운행 현황</h2>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={deptChartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
            ...
            <Bar dataKey="거리" fill="#10b981" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>
